...
function getTransactionType(string $action): int
{
    ...
    return $types[strtolower($action)] ?? 4; // Default to 'other'
}
 ...
 * @param string $action The action being logged (e.g., "Sent file: filename").
 ...
 */
function logActivity(int $userId, string $action, ?int $fileId = null, ?int $departmentId = null, ?int $usersDepartmentId = null): void
{
    global $pdo;
    ...
    ");
    $stmt->execute([$userId, $usersDepartmentId, $fileId, getTransactionType('send'), $action]);
}
...
function logFileActivity(int $userId, string $fileName, int $fileId, array $departmentIds = []): void
{
    ...
    if (empty($departmentIds)) {
        logActivity($userId, "Sent file: $fileName", $fileId, null, $usersDepartmentId);
    } else {
        ...
        foreach ($departmentIds as $departmentId) {
            ...
            logActivity($userId, "Sent file: $fileName to department: $departmentName", $fileId, $departmentId, $usersDepartmentId);
        }
    }
}
